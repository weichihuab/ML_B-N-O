...
cohesive = data[0]["cohesive_energy"]
...
for i in range(len(data)):
  ...
  label_list.append(data[i]["cohesive_energy"])
...
grid_search = GridSearchCV(RandomForestRegressor(random_state=88), param_grid, cv=10)
tree = grid_search.fit(X_trainval, y_trainval)
# save model using joblib
import joblib
joblib.dump(tree, 'rf_cohesive.joblib')
...
plt.plot([0,10],[0,10],color='r',linestyle='dashed')
plt.scatter(pred_tree, y_test,edgecolors='b',alpha=0.4 )
#plt.title("Bulk Modulus")
plt.xlabel("Predicted value (eV/atom)", fontsize=12)
plt.ylabel("DFT value (eV/atom)", fontsize=12)
plt.text(5.2,7.0,target, fontsize=12)
#plt.text(350,50,"$p$ = {:.3f}".format(np.corrcoef(pred_tree, y_test)[0, 1]), fontsize=12)
plt.text(6.25,4.9,"$r$ = {:.3f}".format(np.corrcoef(pred_tree, y_test)[0, 1]), fontsize=12)
plt.xlim((4.25, 7.25))
plt.ylim((4.25, 7.25))
